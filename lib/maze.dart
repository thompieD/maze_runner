...
class Maze extends Component {
  ...
  Future<void> onLoad() async {
    ...
    dungeon = List.generate(height, (_) => List.generate(width, (_) => 1));
 ...
 void _generateDungeon() {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      if (dungeon[y][x] == 1 && random.nextBool()) {
        dungeon[y][x] = 2; 
      }
    }
  }
...
//todo: find a better spritesheet for the corners. 
void addWallTile(int x, int y) {
  ...
  for (int dy = -1; dy <= 1; dy++) {
    for (int dx = -1; dx <= 1; dx++) {
      if (x + dx >= 0 && x + dx < width && y + dy >= 0 && y + dy < height && dungeon[y + dy][x + dx] == 0) {
        adjacentToPath = true;
        break;
      }
    }
  ...
  bool left = (x > 0 && dungeon[y][x - 1] == 0);
  bool right = (x < width - 1 && dungeon[y][x + 1] == 0);
  bool top = (y > 0 && dungeon[y - 1][x] == 0);
  bool bottom = (y < height - 1 && dungeon[y + 1][x] == 0);

  bool topLeft = (x > 0 && y > 0 && dungeon[y - 1][x - 1] == 0);
  bool topRight = (x < width - 1 && y > 0 && dungeon[y - 1][x + 1] == 0);
  bool bottomLeft = (x > 0 && y < height - 1 && dungeon[y + 1][x - 1] == 0);
  bool bottomRight = (x < width - 1 && y < height - 1 && dungeon[y + 1][x + 1] == 0);
